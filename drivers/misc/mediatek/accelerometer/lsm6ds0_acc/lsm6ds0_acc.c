...
#define LSM6DS0_ACC_MTK_NEW_SENSOR_ARCH
...
static int lsm6ds0_acc_init_flag = -1;
static DEFINE_MUTEX(lsm6ds0_acc_gsensor_mutex);
...
static int lsm6ds0_acc_local_uninit(void);
...
static struct acc_init_info lsm6ds0_acc_init_info = {
	...
	.uninit = lsm6ds0_acc_local_uninit,
};
/*----------------------------------------------------------------------------*/
...
static int lsm6ds0_acc_local_init(void)
{
	printk("dean add lsm6ds0_acc local init enter!\n"); 
    ...
    {       
            GSE_ERR("lsm6ds0_acc init flag error\n");
            return -1;
    }
	//GSE_ERR("lsm6ds0_acc_local_init exit \n");
	printk("deans add lsm6ds0_acc_local_init exit!\n");
...
static int LSM6DS0_ACC_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		#if 1
		obj->data[LSM6DS0_ACC_AXIS_X] = obj->data[LSM6DS0_ACC_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		obj->data[LSM6DS0_ACC_AXIS_Y] = obj->data[LSM6DS0_ACC_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		obj->data[LSM6DS0_ACC_AXIS_Z] = obj->data[LSM6DS0_ACC_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;		
		#endif
...
static int lsm6ds0_acc_enable_nodata(int en)
{
    ...
    mutex_lock(&lsm6ds0_acc_gsensor_mutex);
	...
	else
	{
		...
		if (atomic_read(&obj_i2c_data->suspend) == 0)
		{
			...
			err = LSM6DS0_ACC_SetPowerMode(obj_i2c_data->client, enable_status);
	...
	mutex_unlock(&lsm6ds0_acc_gsensor_mutex);

    if(err != LSM6DS0_ACC_SUCCESS)
	{
		printk("LSM6DS0_ACC_gsensor_enable_nodata fail!\n");
		return -1;
	}

    printk("LSM6DS0_ACC_gsensor_enable_nodata OK!!!\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
static int lsm6ds0_acc_set_delay(u64 ns)
{
	...
	{	
		...
		obj_i2c_data->fir.sum[LSM6DS0_ACC_AXIS_X] = 0;
		obj_i2c_data->fir.sum[LSM6DS0_ACC_AXIS_Y] = 0;
		obj_i2c_data->fir.sum[LSM6DS0_ACC_AXIS_Z] = 0;
...
static int lsm6ds0_acc_get_data(int* x ,int* y,int* z, int* status)
{
    ...
    //GSE_FUN();
   	mutex_lock(&lsm6ds0_acc_gsensor_mutex);
	LSM6DS0_ACC_ReadSensorData(obj_i2c_data->client, buff, 256);
	mutex_unlock(&lsm6ds0_acc_gsensor_mutex);
...

{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			#else
				cali[LSM6DS0_ACC_AXIS_X] = sensor_data.x;
				cali[LSM6DS0_ACC_AXIS_Y] = sensor_data.y;
				cali[LSM6DS0_ACC_AXIS_Z] = sensor_data.z;			  
		...
		case GSENSOR_IOCTL_GET_CALI:
			...
			#else
			sensor_data.x = cali[LSM6DS0_ACC_AXIS_X];
			sensor_data.y = cali[LSM6DS0_ACC_AXIS_Y];
			sensor_data.z = cali[LSM6DS0_ACC_AXIS_Z];
...
#ifdef CONFIG_COMPAT
static long LSM6DS0_ACC_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
	...
	switch(cmd)
	{
		...
		case COMPAT_GSENSOR_IOCTL_READ_OFFSET:
			...
			if(err < 0)
			{
				GSE_ERR("COMPAT_GSENSOR_IOCTL_READ_OFFSET is failed!\n");
			}				 
			break;
...
static int LSM6DS0_ACC_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int err = 0;
	GSE_FUN();
#ifdef LSM6DS0_ACC_MTK_NEW_SENSOR_ARCH
...
//	client->addr = 0x6a;
	...
	GSE_LOG("LSM6DS0_ACC_device register ok\n");
#ifdef LSM6DS0_ACC_MTK_NEW_SENSOR_ARCH
	...
	ctl.open_report_data= lsm6ds0_acc_open_report_data;	//interrupt mode
